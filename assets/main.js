 ...
 function checkCollision (){
  if (
    ...
    if (
      ...
      player.velocity.y = 0; // Stop falling
      player.position.y = goomba.position.y - player.height; // Snap to obstacle top
      marioIdle = marioDeath;
    }

    if (
      player.velocity.y < 0 && // Jumping
      player.position.y >= goomba.position.y + goomba.height &&
      player.position.y + player.velocity.y <= goomba.position.y + goomba.height
    ) {
      player.velocity.y = 0; // Stop upward movement
      player.position.y = goomba.position.y + goomba.height; // Snap below obstacle
    }
  }
  if (
    player.position.y + player.height > goomba.position.y &&
    ...
    if (
      player.velocity.x > 0 && // Moving right
      player.position.x + player.width <= goomba.position.x &&
      player.position.x + player.width + player.velocity.x >= goomba.position.x
    ) {
      player.velocity.x = 0; // Stop moving right
      player.position.x = goomba.position.x - player.width; // Snap to goomba left
    }

    if (
      player.velocity.x < 0 && // Moving left
      player.position.x >= goomba.position.x + goomba.width &&
      player.position.x + player.velocity.x <= goomba.position.x + goomba.width
    ) {
      player.velocity.x = 0; // Stop moving left
      player.position.x = goomba.position.x + goomba.width; // Snap to goomba right
    }
  }
}
